...
import { Check, Loader2, Phone, Mail, FileText, Handshake, ClipboardCheck, BadgeCheck, Send } from 'lucide-react';
          ...
          <ol className="grid md:grid-cols-4 gap-px bg-ink/12 border border-ink/12 rounded-none overflow-hidden mt-12">
            {e.steps.map((s: any, i: number) => {
              ...
              <Reveal key={i} as="li" delay={0.08 * i} className="bg-salt p-7 md:p-8">
                <div className="flex items-center justify-between"><span className="font-display text-saffron text-4xl font-medium">{s.number}</span><span className="w-11 h-11 rounded-full bg-ink text-saffron flex items-center justify-center"><Ico size={18} /></span></div>
      ...
      <section id="formulaire" className="section bg-ink text-salt on-dark grain relative overflow-hidden scroll-mt-16">
        <div className="wrap grid lg:grid-cols-12 gap-12">
            ...
            <Reveal delay={0.2} className="mt-10 space-y-6">
              ...
              <div className="card-dark p-6">
                ...
                <div className="space-y-2 text-[15px]">
                  ...
                  <a href={SITE.whatsappHref} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 hover:text-saffron transition-colors"><WhatsAppIcon size={15} /> {SITE.mobile} (WhatsApp)</a>
          ...
          <Reveal className="lg:col-span-6 lg:col-start-7" delay={0.15}>
            <div className="card-dark p-7 md:p-10 backdrop-blur-sm">
              {state === 'done' ? (
                <div className="py-10 text-center">
                  ...
                  <p className="t-body text-sea mt-3 max-w-[40ch] mx-auto">{e.successDesc}</p>
                ...
                <form onSubmit={submit} className="grid sm:grid-cols-2 gap-5">
                  ...
                  <div className="sm:col-span-2 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <Button type="submit" variant="saffron" size="lg" disabled={state === 'sending'} icon="none">{state === 'sending' ? <Loader2 className="animate-spin" size={18} /> : <><Send size={16} /> {e.submitBtn}</>}</Button>
                  </div>
                </form>
              )}
            </div>
